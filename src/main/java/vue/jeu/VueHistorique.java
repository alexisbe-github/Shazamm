...
public class VueHistorique extends JFrame {
	...
	private JPanel tableauPrincipal = new JPanel(); // Tableau de une colonne dans lequel on placera Un label (manche x
													// tour x) par tour et un panel contenant les infos
	...
	private JPanel getEtatPont(TourSQL t) {
		...
		for (int i = 0; i < Pont.TAILLE_PONT; i++) {
			if (i <= numeroManche - 1 || i >= Pont.TAILLE_PONT - numeroManche) {
				// Condition à remplacer par : SI i == indexLave-1 || SI i ==
				// TAILLE_PONT-indexLave+1
	...
	private String getBilanJoueur(JoueurSQL j, TourSQL t) {
		...
		if (j.getId() == partie.getIdJoueur1()) {
			texteBilan = String.format("<html>Mise de %s : %s.<br/>", j.getNom(),
					((Integer) t.getMiseJoueur1()).toString());
		} else {
			texteBilan = String.format("<html>Mise de %s : %s.<br/>", j.getNom(),
					((Integer) t.getMiseJoueur2()).toString());
		}
