 ...
 * {@link main.java.model.dao.beans.CouleurSQL CouleurSQL}, spécifiquement pour la
 * table <code><i>couleur</i></code> de la base de données, qui enregistre les
 * informations relatives aux couleurs.
 * 
 * 
...
public class DAOCouleur extends DAO<CouleurSQL> {
	...
	/**
	 * Table <code><i>couleur</i></code>, contenant différentes informations sur les couleurs.
	 */
	...
	/**
	 * Colonne <code><i>couleur_j1</i></code>, correspondant à la couleur du couleur 1.
	 */
	private final String COULEUR_J1 = "couleur_j1";
	/**
	 * Colonne <code><i>couleur_j2</i></code>, correspondant à la couleur du couleur 2.
	 */
	 ...
	 * @return Les couleurs des joueurs de la partie correspondant à l'identifiant passé en paramètre
	 * 
	 */
	@Override
		...
		try (Connection connexion = this.connexion.getConnexion();
			...
			try (ResultSet rs = pstmt.getResultSet()) {
				if (rs.first()) {
					...
					couleur.setCouleurJ1(rs.getString(COULEUR_J1).toLowerCase().equals("vert") ? ECouleurJoueur.VERT : ECouleurJoueur.ROUGE);
					couleur.setCouleurJ2(rs.getString(COULEUR_J2).toLowerCase().equals("rouge") ? ECouleurJoueur.ROUGE : ECouleurJoueur.VERT);
				}
			}
	...
	public CouleurSQL creer(CouleurSQL couleur) {
		try (Connection connexion = this.connexion.getConnexion();
				PreparedStatement pstmt = connexion
						.prepareStatement("INSERT INTO " + COULEUR + " VALUES (?, ?, ?);")) {
	...
	public void supprimer(CouleurSQL couleur) {
		try (Connection connexion = this.connexion.getConnexion();
				PreparedStatement pstmt = connexion.prepareStatement("DELETE FROM " + COULEUR + " WHERE " + ID + " = ?;")) {
			pstmt.setLong(1, couleur.getIdPartie());
			pstmt.execute();
		} catch (SQLException e) {
			e.printStackTrace();
